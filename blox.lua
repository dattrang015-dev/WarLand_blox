...
local TargetParent = player:WaitForChild("PlayerGui")-- [[ 🚀 WARLAND VN - V102.1: SMOOTH FLY-FARM & SYNTAX FIX (FIXED) ]]
...
hitboxPart.Size = Vector3.new(19, 19, 19)
		...
		if _G.AutoFarmLevel and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
			...
			if not hasQuest then
				...
				if targetMob and targetMob:FindFirstChild("HumanoidRootPart") then
					...
					-- Bay ngay trên đầu quái
					_G.FlyTarget = CFrame.new(mobPos + Vector3.new(0, 8, 0))
					_G.FlyStopDistance = 3

					-- Kiểm tra quái trong cube hitbox 19x19x19 (AABB)
					local diff = mobPos - hrp.Position
					local inHitbox = math.abs(diff.X) <= 9.5 and math.abs(diff.Y) <= 9.5 and math.abs(diff.Z) <= 9.5
					...
					if inHitbox then
						...
						-- Gây damage trực tiếp trên client (ko cần server)
						local COMBO_DAMAGE = {8, 10, 12, 15}
						local dmg = COMBO_DAMAGE[comboCount] or 8
						local halfSize = 9.5
						for _, obj in ipairs(workspace:GetDescendants()) do
							if obj:IsA("Humanoid") and obj.Health > 0 then
								local parent = obj.Parent
								if parent ~= player.Character then
									local tHrp = parent:FindFirstChild("HumanoidRootPart")
									if tHrp then
										local d = tHrp.Position - hrp.Position
										if math.abs(d.X) <= halfSize and math.abs(d.Y) <= halfSize and math.abs(d.Z) <= halfSize then
											obj:TakeDamage(dmg)
										end
									end
								end
...
-- [ LOGIC FLY SMOOTH (BodyVelocity/BodyGyro + RunService.Heartbeat) ]
RunService.Heartbeat:Connect(function(dt)
	if _G.Flying and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
		...
		if _G.FlyTarget then
			-- Chế độ Auto Farm: Bay đến target (smooth)
			...
			if dist > stopDist then
				-- Giảm tốc mượt khi gần target
				local speedFactor = math.min((dist - stopDist) / 30, 1)
				local targetVel = dir.Unit * _G.FlySpeed * speedFactor
				smoothVelocity = smoothVelocity:Lerp(targetVel, math.min(dt * 8, 1))
				flyBV.Velocity = smoothVelocity
			else
				-- Đã đến → lơ lửng mượt (thêm nhẹ chống trọng lực)
				smoothVelocity = smoothVelocity:Lerp(Vector3.new(0, 0.5, 0), math.min(dt * 10, 1))
				flyBV.Velocity = smoothVelocity
			end

			-- Xoay mặt hướng target (smooth, không snap)
			local lookTarget = Vector3.new(targetPos.X, hrp.Position.Y, targetPos.Z)
			if (lookTarget - hrp.Position).Magnitude > 0.1 then
				local lookCFrame = CFrame.new(hrp.Position, lookTarget)
				flyBG.CFrame = flyBG.CFrame:Lerp(lookCFrame, math.min(dt * 8, 1))
			end
		else
			-- Chế độ tự do: Bay theo camera
			if humanoid and humanoid.MoveDirection.Magnitude > 0 then
				local targetVel = Camera.CFrame.LookVector * _G.FlySpeed
				smoothVelocity = smoothVelocity:Lerp(targetVel, math.min(dt * 8, 1))
				flyBV.Velocity = smoothVelocity
			else
				smoothVelocity = smoothVelocity:Lerp(Vector3.new(0, 0.5, 0), math.min(dt * 10, 1))
			...
			flyBG.CFrame = flyBG.CFrame:Lerp(Camera.CFrame, math.min(dt * 8, 1))
		end
	else
		-- Tắt fly: dọn dẹp
...
RunService.Heartbeat:Connect(function()
	...
	if hrp and _G.AutoFarmLevel then
